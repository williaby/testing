...
Key Components:
    - CloudflareUser: Represents an authenticated user
...
class CloudflareUser(BaseModel):
    ...
    extracted from Cloudflare Access authentication headers.

    Attributes:
        email: User's email address
        user_id: Unique user identifier from JWT subject
        claims: Full JWT claims object
        authenticated_at: When the user was authenticated (current time)
    ...
    @classmethod
    def from_jwt_claims(cls, claims: CloudflareJWTClaims) -> "CloudflareUser":
        """Create CloudflareUser from JWT claims.
